    ...
    <q-card id="loginDialog" style="min-width: 350px">
      ...
      <q-card-section>
        <q-form @submit="submit()" @reset="user.loginDialog.reset()">
          <q-input filled type="email" v-model="user.loginDialog.email" label="E-mail" lazy-rules
                   :rules="[val => !!val]"/>
          <q-input filled v-model="user.loginDialog.password" label="Password"
                   :type="user.loginDialog.pwdVisible ? 'text' : 'password'" lazy-rules :rules="[val => !!val]">
            <template v-slot:append>
              <q-icon :name="user.loginDialog.pwdVisible ? 'visibility' : 'visibility_off'" class="cursor-pointer"
                      @click="user.loginDialog.pwdVisible = !user.loginDialog.pwdVisible"/>
            </template>
          </q-input>
...
<script>
import {user} from '/store/user.mjs'
